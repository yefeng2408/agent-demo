...
public class SupportDeltaStrategy implements DeltaStrategy {
    ...
    public List<ClaimDelta> apply(EpistemicContext ctx) {
        ...
        String claimKey = keyCodec.buildCitationKey(dominant);
        double before = dominant.confidence();
        // ===== repeated assertion（衰减支持）=====
        if (ctx.isRepeatedAssertion(ctx, dominant)) {

            double after = clamp(before + deltaComputer.computeStep(before) * 0.40);

            ClaimDelta delta = ClaimDelta.confidenceOnly(
                    claimKey,
                    before,
                    after,
        ...
        // ===== 正常 support =====
        double deltaValue = deltaComputer.computeStep(before);
        double after = clamp(before + deltaValue);
